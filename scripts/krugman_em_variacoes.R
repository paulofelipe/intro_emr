...
krugman_df <- expand.grid(
  ...
  mutate(Q0_s = Y0/P0) %>% 
  ungroup() %>% 
  mutate(pf0 = 1/(1 - 1/sig),
         qf0 = vx0/(pf0 * N0),
         tau = (1 - 1/sig) * pf0/c0) %>% 
  group_by(goods, exporter) %>% 
  mutate(fc_share = fc/(mean(fc) + sum(qf0)),
         qf_share =  qf0/sum(qf0))
...
params[["fc_share"]] <- create_param(
  value = krugman_df %>% 
    select(goods, regions = exporter, fc_share) %>% 
    distinct(),
  indexes = sets[c('goods', 'regions')],
  desc = 'fixed costs share'
)


params[["qf_share"]] <- create_param(
  value =  krugman_df %>% 
    select(goods, exporter, importer, qf_share),
  indexes = sets[c('goods', 'exporter', 'importer')],
...
equations[["P"]] <- create_equation(
  'P[k,s] = sum(vx0[k,,s]/sum(vx0[k,,s]) * N[k,]*PF[k,,s]^(1-sig))^(1/(1-sig))',
...
equations[["c"]] <- create_equation(
  'c[k,r] = sum(vx0[k,r,]/sum(vx0[k,r,]) * PF[k,r,] * QF[k,r,])',
  indexes = c('k in goods', 'r in regions'),
  type = "defining",
  desc = "composite input price (marginal cost)"
)
...
equations[["N"]] <- create_equation(
  'N[k,r] - Y[k,r]/(fc_share[k,r] + (1-fc_share[k,r]) * sum(qf_share[k,r,] * tau[k,r,] * QF[k,r,]))',
  indexes = c('k in goods', 'r in regions'),
  type = "mcc",
  desc = "number of firms (varieties)"
)
...
krugman$params$tau$value['G1', 'R1', 'R2'] <- 0.6363961

system.time(
  sol <- solve_emr_block(krugman, trace = TRUE, tol = 1e-10,
                         alpha_min = 0.2, alpha_max = 0.2)
)
