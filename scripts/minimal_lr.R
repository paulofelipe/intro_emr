minimal_lr <- function(use_df, fac_df, tar_df, ptx_df,
  ...
  variables[["x3tot"]] <- create_variable(
    value = 1,
    indexes = "x3tot",
    type = "undefined",
    desc = "Variação no dispêndio real das famílias"
  ...
  equations[["E_x3tot"]] <- create_equation(
    'delB',
    type = "mcc",
    desc = "Variação na renda (dispêndio) real das família"
  )
  ...
  equations[["E_rw"]] <- create_equation(
    'rw = p1lab/p3tot',
    type = "defining",
    desc = "Variação no salário real"
  )
  ...
  equations[["E_p1lab"]] <- create_equation(
    ...
    type = "mcc",
    desc = "Equilíbrio no mercado de trabalho"
  )
  
  SHARECAP <- fac_df %>%
    filter(FAC == "Capital") %>% 
    mutate(SHARECAP = Value/sum(Value)) %>% 
    select(IND, SHARECAP)
  
  params[["SHARECAP"]] <- create_param(
    value = SHARECAP,
    indexes = sets["IND"],
    desc = "Part. de cada indústria no uso do fator capital"
  )
  
  params[["K"]] <- create_param(
    value = 1,
    indexes = "capital",
    desc = "Variação no estoque de capital"
  )
  ...
  variables[["p1cap"]] <- create_variable(
    ...
    indexes = sets['IND'],
    type = "undefined",
    desc = "Variação na remuneração do capital"
  )
  ...
  equations[["E_p2tot_p"]] <- create_equation(
    "p2tot_p = sum(USE_S[,'Investment'] * p_s[,'Investment'] * x_s[,'Investment'])/sum(USE_S[,'Investment'] * x_s[,'Investment'])",
    type = "defining",
    desc = "Variação no índice de preço do investimento (LaspeyreS)"
  )
