...
equations[["E_delB"]] <- create_equation(
  "delB = sum(sum(USE[,'dom','Exports']) * p4tot * x4tot - 
    ...
    sum(USE[,'dom','Exports'] - V0CIF)/(V0GDPEXP) + 1",
  type = "defining",
  desc = "Variação no índice de preços da importação"
)
```
...
Verifica-se um impacto de +0,86% no PIB real e +1,14% no emprego. O aumento da demanda interna é atendido via redução das exportações, que tiveram redução no volume (-20,46%) e aumento nos preços (+4,90%), e aumento das importações (+13,08%).

```{r}
map_df(runif(10, 0.5,  7), ~{
