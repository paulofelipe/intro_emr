...
A estrutura de produção utilizada no minimal é apresentada na Figura \ref{producao} A estrutura adotada considera um primeiro nível em que o produtor demanda bens intermediários (*commodities*) e o fator primário, que é uma combinação de capital e trabalho. É assumida uma tecnologia do tipo Leontief. Dessa forma, pode-se definir o primeiro nível da produção como:
...
```{r, echo=FALSE, fig.cap="Estrutura da Produção\\label{producao}",fig.pos="H"}
...
$$\hat{\text{COST}}_i = \frac{\sum_c\sum_s USE_{csi} \times \hat{p}_{cs} + FAC_{labour,i} \times \hat{\text{P1LAB}}_i + FAC_{capital,i} \times \hat{\text{P1CAP}_i}}{\text{V1TOT}_i}$$
...
equations[["E_cost"]] <- create_equation(
  "cost[i] = (sum(USE[,,i]*p) +
  FAC0['Labour',i]*p1lab +
  FAC0['Capital',i]*p1cap[i])/V1TOT[i]",
  indexes = c("i in IND"),
  type = "defining",
